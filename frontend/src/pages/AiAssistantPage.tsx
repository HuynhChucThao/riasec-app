...
export const AiAssistantPage: React.FC<AiAssistantPageProps> = ({ initialContext }) => {
  const { user } = useAuth();
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      console.warn('Backend AI offline, using smart rule-based guidance fallback:', err);
  ...
  function generateSmartFallbackReply(
    ...
    if (q.includes('ria') || (ctx?.riasecCode && ctx.riasecCode.includes('R'))) {
    ...
    if (q.includes('kỹ sư') || q.includes('công nghệ') || q.includes('phần mềm')) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 150px)' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 12, borderBottom: '1px solid var(--border-color)', marginBottom: 12 }}>
        <div
          style={{
            width: 40,
            height: 40,
        ...
        <div>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 800, fontFamily: 'var(--font-heading)' }}>
      ...
      {messages.length <= 2 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
            <Sparkles size={12} color="var(--primary-teal)" /> Gợi ý câu hỏi nhanh:
          </span>
      ...
      {/* Chat Messages */}
      <div className="chat-messages" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {messages.map((msg) => {
            ...
            >
              <div
                style={{
                  ...
                  height: 30,
                  borderRadius: '50%',
                  background: isUser ? 'var(--primary-blue)' : 'var(--primary-teal)',
                  color: 'white',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 8, alignSelf: 'flex-start' }}>
            ...
            <div className="message-bubble assistant" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span className="pulse-dot" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--primary-teal)' }} />
              <span className="pulse-dot" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--primary-teal)', animationDelay: '0.2s' }} />
              <span className="pulse-dot" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--primary-teal)', animationDelay: '0.4s' }} />
            </div>
          </div>
        )}
